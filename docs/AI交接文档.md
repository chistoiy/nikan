...
- [x] ~~C5 `build_apk.bat` 路径被写坏（`\a` → 0x07 控制字符）~~ · ~~B2 `_indexStep` 通知条件写错~~
      —— 已记入 `代码审查与功能建议-2026-09-15.md`，待排期（都不影响功能，属清理项）
- [x] **连接稳定性第一轮**（§19）：超时不再必然作废连接、迟到响应清理、事务号自检、
      保活事件驱动、自动重连覆盖相机释放窗口、失败结论可行动、AF 改用 WMU 的 0x9125
- [ ] **§19.4 复验**：空闲 5~10 分钟不再假掉线；取景对焦日志出现 `AF 驱动 0x9125 记录为可用`
      （若被拒，把响应码发回，见 §19.4 第 5 条的三种分支）
...
- [ ] 全部验证通过后提交（U1 + 遥控页 + 断点续传 + §19 连接稳定性，当前未提交）
...
- [ ] **A1 USB 下载独占事务锁**（`PtpUsbClient.getObjectToStream` 全程持锁 → 保活/缩略图饿死）：
      见审查文档 A1，建议在 U1 提交前处理
